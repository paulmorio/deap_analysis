...
for X,y in zip(st_X, st_y):
	...
	SVM_rbf_valence = GaussianNB()
	print "F1 SCORES \n"
	a = cross_val_score(SVM_rbf_valence, pca_result, y_valence, cv= len(y_valence), scoring = 'f1', n_jobs=-1)
	print ((np.mean(a)), (np.std(a)))

	print "\n ACCURACY SCORES \n"
	# Create and fit the Model using the training data
	a = cross_val_score(SVM_rbf_valence, pca_result, y_valence, cv= len(y_valence), scoring = 'accuracy', n_jobs=-1)
	print ((np.mean(a)), (np.std(a)))

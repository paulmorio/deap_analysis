...
# SVM_rbf = svm.SVC(kernel='linear', C= (bestC), gamma= (bestG)) # For RBF Kernel
...
test_size = int(math.ceil(len(X)/32))
X_train = X[indices[:-test_size]]
y_train = y_arousal[indices[:-test_size]]
X_test = X[indices[-test_size:]]
y_test = y_arousal[indices[-test_size:]]
...
test_size = int(math.ceil(len(X)/32))
X_train = X[indices[:-test_size]]
y_train = y_dominance[indices[:-test_size]]
X_test = X[indices[-test_size:]]
y_test = y_dominance[indices[-test_size:]]
